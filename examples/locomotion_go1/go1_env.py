...
GO1_URDF = "urdf/go1_description/urdf/go1.urdf"
EXPECTED_GO1_DOFS = 18
FLOATING_BASE_DOFS = 6
...
class Go1Env:
    ...
    def _validate_robot_config(self):
        if self.robot.n_dofs != EXPECTED_GO1_DOFS:
            raise RuntimeError(f"Expected Go1 to have {EXPECTED_GO1_DOFS} DOFs, got {self.robot.n_dofs}.")

        if self.env_cfg["num_actions"] != len(self.env_cfg["joint_names"]):
            raise RuntimeError(
                f"num_actions ({self.env_cfg['num_actions']}) must match joint_names "
                f"({len(self.env_cfg['joint_names'])})."
            )

        available_joint_names = {joint.name for joint in self.robot.joints}
        missing_joint_names = [name for name in self.env_cfg["joint_names"] if name not in available_joint_names]
        if missing_joint_names:
            raise RuntimeError(
                f"Go1 URDF is missing configured motor joints: {missing_joint_names}. "
                f"Available joints: {sorted(available_joint_names)}."
            )

        merged_motor_joint_names = [joint.name for joint in self.robot.joints[1:]]
        missing_default_angles = [
            name for name in merged_motor_joint_names if name not in self.env_cfg["default_joint_angles"]
        ]
        if missing_default_angles:
            raise RuntimeError(f"default_joint_angles is missing entries for: {missing_default_angles}.")

        invalid_default_angles = []
        for name in self.env_cfg["joint_names"]:
            joint = self.robot.get_joint(name)
            if joint.n_dofs != 1:
                raise RuntimeError(f"Expected motor joint '{name}' to have 1 DOF, got {joint.n_dofs}.")

            lower, upper = joint.dofs_limit[0]
            value = self.env_cfg["default_joint_angles"][name]
            if value < lower or value > upper:
                invalid_default_angles.append((name, value, float(lower), float(upper)))

        if invalid_default_angles:
            details = ", ".join(
                f"{name}={value} outside [{lower}, {upper}]"
                for name, value, lower, upper in invalid_default_angles
            )
            raise RuntimeError(f"Go1 default_joint_angles exceed joint limits: {details}.")
    ...
    def step(self, actions):
        ...
        self.robot.control_dofs_position(
            target_dof_pos[:, self.actions_dof_idx], slice(FLOATING_BASE_DOFS, EXPECTED_GO1_DOFS)
        )
        self.scene.step()
