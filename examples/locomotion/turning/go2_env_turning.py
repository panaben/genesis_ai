...
class Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, record=False):
        ...
        # If set, this command is used instead of random command resampling.
        self.command_override: torch.Tensor | None = None
    ...
    def set_command_override(self, command):
        if command is None:
            self.command_override = None
            return

        command_tensor = torch.as_tensor(command, dtype=gs.tc_float, device=gs.device).reshape((self.num_commands,))
        lower, upper = self.commands_limits
        command_tensor = torch.minimum(torch.maximum(command_tensor, lower), upper)
        self.command_override = command_tensor
        self.commands[:] = self.command_override
    ...
    def _resample_commands(self, envs_idx):
        ...
        if self.command_override is not None:
            if envs_idx is None:
                ...
                self.commands[envs_idx] = self.command_override
            return
    ...
    def step(self, actions):
        ...
        # resample commands
        if not self.is_motion_paused:
            is_resample_step = self.episode_length_buf % int(self.env_cfg["resampling_time_s"] / self.dt) == 0
            self._resample_commands(is_resample_step)
        else:
            self.commands.zero_()
    ...
    def _reset_idx(self, envs_idx=None):
        ...
        # random sample command upon reset (or command override if active)
